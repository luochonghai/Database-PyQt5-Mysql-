...
class SumFunc(QWidget): 
    def __init__(self, parent=None):  
        ...
        modify_show = ["各月总和","费控指标"]
        sql_2 = "select sum(设计费) from final where 项目编号 = '%s' group by 数据类型;"
        sql_3 = "select sum(自行) from final where 项目编号 = '%s' group by 数据类型;"
        sql_4 = "select sum(内部分包) from final where 项目编号 = '%s' group by 数据类型;"
        sql_5 = "select sum(外部分包) from final where 项目编号 = '%s' group by 数据类型;"
        sql_6 = "select sum(现场费用) from final where 项目编号 = '%s' group by 数据类型;"
        sql_7 = "select sum(大临) from final where 项目编号 = '%s' group by 数据类型;"
        sql_8 = "select sum(差旅费) from final where 项目编号 = '%s' group by 数据类型;"
        sql_9 = "select sum(办公费) from final where 项目编号 = '%s' group by 数据类型;"
        sql_10 = "select sum(业务招待费) from final where 项目编号 = '%s' group by 数据类型;"
        sql_11 = "select sum(车辆费用（油料）) from final where 项目编号 = '%s' group by 数据类型;"
        sql_12 = "select sum(零星材料) from final where 项目编号 = '%s' group by 数据类型;"
        sql_13 = "select sum(房租及后勤费用) from final where 项目编号 = '%s' group by 数据类型;"
        sql_14 = "select sum(其他) from final where 项目编号 = '%s' group by 数据类型;"
        sql_15 = "select sum(财务费用) from final where 项目编号 = '%s' group by 数据类型;"
        sql_16 = "select sum(税费) from final where 项目编号 = '%s' group by 数据类型;"
        sql_17 = "select sum(设备采购) from final where 项目编号 = '%s' group by 数据类型;"
        sql_18 = "select sum(分包) from final where 项目编号 = '%s' group by 数据类型;"
        ...
        try:
            ...
            if modifyTemp[1] not in pj_name:
                ...
                for j in range(2):
                    for i in range(17):
                        sql_now = "sql_"+str(i+2)
                        result = temp.execute(eval(sql_now)%(modifyTemp[1]))
                        result = temp.fetchall()
                        sql_sum = 'sum('+sum_list[i+2]+')'
                        """notice here:only type string can be shown on the QTableWidgetItem"""
                        sumdata[i][j] = str(result[j][sql_sum])

                self.MyTable = QTableWidget(2,19)  
